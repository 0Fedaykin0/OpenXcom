...
void NewManufactureListState::fillProductionList(bool refreshCategories)
{
	...
	for (const auto* manuf : _possibleProductions)
	{
		if ((manuf->getCategory() == _catStrings[_cbxCategory->getSelected()]) || (_catStrings[_cbxCategory->getSelected()] == "STR_ALL_ITEMS"))
		{
			...
			// amount owned
			int amount_owned = 0;
			for (std::map<const RuleItem*, int>::const_iterator pitem = manuf->getProducedItems().begin(); pitem != manuf->getProducedItems().end(); ++pitem)
			{
			...
			_lstManufacture->addRow(4, tr(manuf->getName()).c_str(), tr(manuf->getCategory()).c_str(), ss.str().c_str(), amount_owned_str.str().c_str());
